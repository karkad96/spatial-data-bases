...
--14
	...
	--b)
	CREATE USER Superuser283109 WITH PASSWORD '123' SUPERUSER;
	CREATE USER guest283109 WITH PASSWORD '321';
	GRANT CONNECT ON DATABASE s283109 TO guest283109;
	GRANT USAGE ON SCHEMA sklep TO guest283109;
	GRANT SELECT ON ALL TABLES IN SCHEMA sklep TO guest283109;
	SELECT rolname FROM pg_roles;
	--b)
	
	--c)
	ALTER ROLE Superuser283109 RENAME TO student;
	ALTER ROLE student NOSUPERUSER;
	GRANT CONNECT ON DATABASE s283109 TO student;
	GRANT USAGE ON SCHEMA sklep TO student;
	GRANT SELECT ON ALL TABLES IN SCHEMA sklep TO student;
	
	REASSIGN OWNED BY guest283109 TO postgres;
	DROP OWNED BY guest283109;
	DROP USER guest283109;
	--c)
--14
